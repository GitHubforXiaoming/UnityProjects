...
std::unordered_map<int, int> spendTimes;	// 用于存储每个客户端运行时间
std::unordered_map<int, std::string> buildTimes;
...
void processRequest(int number, std::string message)
{
	std::vector<std::string> splits = SplitString(message, "-");
	if (splits.size() > 1)
	{
		if (splits[0] == "build")
		{
			buildTimes[number] = splits[1];
			spendTimes[number] = time(NULL);; 
		}
		if (splits[0] == "done")
		{
			time_t cur = time(NULL);
			if (cur - spendTimes[number] >= 10)
			{
				sendMsg(buildTimes[number].c_str());
			}
		}
	}
}

void sendBackMsg(SOCKET client, char *buf)
{
...
void recvFromClient(SOCKET clientsoc, char *buf, int number)//接受一个客户端的消息并转发给所有客户端
{
	while (1)
	{
		...
		printf("客户端%d发过来的消息:%s", number, buf);
